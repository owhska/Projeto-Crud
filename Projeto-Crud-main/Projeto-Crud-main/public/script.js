...
function _emailValido() {
    ...
    return validarEmail(email); /* se o valor de email existir e estiver true no na funcao de validarEmail retorne */
}
...
function alterarBotoes() {
    const emailValido = _emailValido(); /* constante emialValido recebe o retorno da funcao de _emailvalido*/
    form.recoverPassword().disabled = !emailValido; /* pega o id do botao recuperar senha, para destivar ou ativar o botao*/
    if (!emailValido || form.email().value === "") {
        alert("Email inválido ou vazio");
    }
    
    const senhaValida = _senhaValida(); /* recebe o retorno da funcao de verificacao true or false */
